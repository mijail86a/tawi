	...
	<script type="text/javascript">
		$(function() {
			...
			buscar.on('click', function () {
				...
				envia = inicaAjax(url, { origen: "reporte", start: start, end: end, "estado" : 4 });
				envia.done(function(data) {
					if(data.estado){
						var dynatable = table.dynatable({
					      	...
					      	writers:{
					      		estado: function(el) {
	                                var boton = "<button type=\"button\" class=\"btn btn-danger btn-block\">Rechazado</button>";
	                                return boton;
	                            },
					      		total: function(el) {
		...
		var reporte = function(){
			envia = inicaAjax(url, { origen: "reporte-automatico", "estado" : 4 });
			envia.done(function(data) {
				if(data.estado){
					table.dynatable({
				      	dataset: {
