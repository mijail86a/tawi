	...
	<script type="text/javascript">
		$(function() {
			...
			buscar.on('click', function () {
				...
				envia = inicaAjax(url, { origen: "reporte", start: start, end: end, "estado" : 3 });
				envia.done(function(data) {
					if(data.estado){
						var dynatable = table.dynatable({
					      	...
					      	writers:{
					      		estado: function(el) {
	                                var boton = "<button type=\"button\" class=\"btn btn-success btn-block\">Pagado</button>";
	                                return boton;
	                            },
					      		total: function(el) {
		...
		var reporte = function(){
			envia = inicaAjax(url, { origen: "reporte-automatico", "estado" : 3 });
			envia.done(function(data) {
				if(data.estado){
					table.dynatable({
				      	dataset: {
